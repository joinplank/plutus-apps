...
runConfigCommand _ ConfigCommandArgs{ccaAvailability, ccaTrace, ccaPABConfig=Config { nodeServerConfig, chainIndexConfig = ChainIndexConfig ciConfig }} ChainIndex = do
    params <- liftIO $ Params.fromPABServerConfig nodeServerConfig
    ChainIndex.main
